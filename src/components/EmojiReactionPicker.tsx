...
type FavoriteReactionButtonProps = {
	isFavorite: boolean;
	isDisabled: boolean;
	label: string;
	className: string;
...
function EmojiReactionPicker({
	...
	const searchResults = useMemo(
		() =>
			catalog && !emoji
				? searchEmojiShortcodes(catalog, input)
				: [],
		[catalog, emoji, input],
	);
	...
	const toggleAndClose = async (content: string) => {
		...
		try {
			...
			// The caller reports publishing errors.
		}
	};
	...
	const submit = async () => {
		...
		if (hasReaction(selectedEmoji)) {
			setError("Reaction already added. Remove it from your reactions below.");
			return;
		}
				...
				{input.trim() ? (
					<span className="mt-2 block">
						{isCatalogLoading && !emoji ? (
							...
							<span className="grid grid-cols-6 gap-1" aria-label="Matching emoji">
								<ReactionTile
									symbol={emoji}
									label={hasReaction(emoji)
										? `${emoji} reaction already added`
										: `Add ${emoji} reaction`}
									isActive={hasReaction(emoji)}
									isSaving={isSaving}
									onClick={hasReaction(emoji)
										? null
