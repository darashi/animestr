...
function useWikidataReactionsTimeline(): WikidataReactionItem[] {
	...
	const [reactions, setReactions] = useState<ReactionEvent[]>(() => {
		if (typeof eventStore.getByFilters === "function") {
			const existing = eventStore.getByFilters(reactionFilter) as ReactionEvent[] | undefined;
	...
	const [deletedIds, setDeletedIds] = useState<Set<string>>(() => {
		if (typeof eventStore.getByFilters === "function") {
			const existingDeletes = eventStore.getByFilters(deleteFilter) as ReactionEvent[] | undefined;
	...
	const filtered = useMemo(() => {
		...
		return items.sort((a, b) => (b?.event?.created_at ?? 0) - (a?.event?.created_at ?? 0));
...
export default useWikidataReactionsTimeline;
